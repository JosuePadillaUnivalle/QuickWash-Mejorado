...
<div class="machine-visual {{ $maintenance ? 'maintenance' : '' }}"><span class="status {{ $maintenance || !$start->isFuture() ? 'pendiente' : ($busy ? 'en_proceso' : 'finalizada') }}">{{ $maintenance ? 'Mantenimiento' : (!$start->isFuture() ? 'Horario iniciado' : (in_array($machine->id, $awaitingPickup) ? 'Esperando recogida' : ($busy ? 'Ocupada en este turno' : 'Disponible en este turno'))) }}</span><x-icon class="big-washer"/></div>
...
<div class="inline-note"><x-icon name="info"/><span>Pendiente, En proceso y Esperando recogida cuentan como activas. Una máquina con ropa sin recoger no acepta nuevas reservas. Los turnos ya reservados pueden retrasarse hasta su liberación. Solo puedes cancelar una reserva pendiente antes de que comience su horario.</span></div>
@endsection
